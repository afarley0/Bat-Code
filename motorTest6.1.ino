...
void setup() {
  ...
  stepper_2.setAcceleration(2500);
  stepper_3.setMaxSpeed(5500);
  stepper_3.setAcceleration(2500);
  stepper_4.setMaxSpeed(5500);
  stepper_4.setAcceleration(2500);
  stepper_5.setMaxSpeed(5500);
  stepper_5.setAcceleration(2500);
...
void loop() {
  if (Serial.available() > 0){
    ...
    if (go.equals("go"))
    {
      ...
      boolean runs = true;
      while (runs){
        String willRun = Serial.readStringUntil(';');
        Serial.println(willRun);
        if (willRun.equals("run")){
          neoDynamicControl(1, noseStep, earStep);
        }
        else if (willRun.equals("stop")){
          runs = false;
        }
      }
